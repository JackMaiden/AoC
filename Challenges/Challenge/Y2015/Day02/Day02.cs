...
{
    public async Task<object?> TaskPartOne(string input) => ParseBoxes(input.GetLines()).Sum(s => s.Paper);

    public async Task<object?> TaskPartTwo(string input) => ParseBoxes(input.GetLines()).Sum(s => s.Ribbon);
    ...
    private readonly record struct Box(int L, int W, int H)
    {
        ...
        public int Ribbon => (L * W * H) + _perimeters.Min();
    }

    private IEnumerable<Box> ParseBoxes(IEnumerable<string> input) => input.Select(s =>
    {
